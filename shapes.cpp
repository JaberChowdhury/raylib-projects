...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    DrawTextEx(myfont, "Welcome to my raylib project", {500, 1230}, 120, 1,
               text);
    // --- 1. SQUARE & RECTANGLE ---
    ...
    DrawTextEx(myfont, ("Poly" + std::to_string(x)).c_str(), {1450, 150}, 40, 2,
               text);
    ...
    for (int i = 0; i <= cols; i++) {
      DrawLine(startX + (i * cellSize), startY, startX + (i * cellSize),
               startY + (rows * cellSize), text);
    }
    for (int i = 0; i <= rows; i++) {
      DrawLine(startX, startY + (i * cellSize), startX + (cols * cellSize),
               startY + (i * cellSize), text);
    }
